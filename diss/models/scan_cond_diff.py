...
class ScanCondDiffLatent(LightningModule):
    def __init__(self, hparams:dict):
        ...
        self.latent_diff_ema = LatentCondDiffuser(latent_dim=128, mid_attn=self.hparams['model']['mid_attn'], cond_diff=True)
    ...
    def forward_diff(self, feats, cond_feats, t, ema=False):
        return self.latent_diff_ema(feats, t, cond_feats) if ema else self.latent_diff(feats, t, cond_feats)

    def classfree_forward(self, latent, cond, uncond, t, ema=False):
        if ema:
            x_cond = self.forward_diff(latent, cond, t, ema)
            x_uncond = self.forward_diff(latent, uncond, t, ema)
        else:
            x_cond = self.forward_diff(latent, cond, t, ema)
            x_uncond = self.forward_diff(latent, uncond, t, ema)

        return x_uncond + self.hparams['diff']['w_cond'] * (x_cond - x_uncond)
    ...
    def training_step(self, batch:dict, batch_idx):
        ...
        if torch.rand(1) > self.hparams['diff']['uncond_prob'] or len(batch['coords']) == 1:
            x_cond = points_to_tensor(batch['cond'][0], batch['cond'][1], self.hparams['data']['resolution'], -1)
        else:
            x_cond = points_to_tensor(
               [torch.zeros_like(c) for c in batch['cond'][0]],
               [torch.zeros_like(c) for c in batch['cond'][1]],
               self.hparams['data']['resolution'], -1
            )

        x_cond = self.cond_enc(x_cond)
        pred_noise = self.forward_diff(noisy_latent, x_cond, t)
    ...
    def p_sample_loop(self, x_t, cond, ema=False):
        self.scheduler_to_cuda()
        ...
        for t in tqdm(range(len(self.dpm_scheduler.timesteps))):
            ...
            noise_t = self.classfree_forward(x_t, x_cond, x_uncond, t, ema)
    ...
    def cond_to_pcd(self, gen_pcd, cond):
        pcd_part = o3d.geometry.PointCloud()
        points = cond.cpu().numpy()
        min_part = points.min(0)
        min_gen = np.array(gen_pcd.points).min(0)
        offset_pcd = min_gen - min_part
        points += offset_pcd
        pcd_part.points = o3d.utility.Vector3dVector(points)
        pcd_part.paint_uniform_color([0., 0.,1.])
    ...
    def infer_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            x0_pred = self.p_sample_loop(noise_in, batch['cond'], ema=False)
            ...
            for i in range(bsize):
                ...
                np.save(f'{self.logger.log_dir}/generated_pcd/x0/{batch_idx*bsize + i}.npy', pcd_x0)

                pcd_gt = np.concatenate((batch['coords'][i].cpu().numpy(), batch['feats'][i].cpu().numpy()), axis=-1)
                pcd_gt[:,:3] = self.devoxelize(pcd_gt[:,:3])
                np.save(f'{self.logger.log_dir}/generated_pcd/real_data/{batch_idx*bsize + i}.npy', pcd_gt)
